...
- [x] Cài đặt và bọc app bằng `<BrowserRouter>`.
- [x] Khai báo các route chính: `/` (Home), `/projects` (List), `/projects/:id` (Details), `/saved` (Saved), `/admin` (Protected), `*` (NotFound).
- [x] Viết `ProtectedRoute` kiểm tra quyền (ví dụ giả lập auth: `isLoggedIn`).

**ProjectDetailPage [B]**
- [x] Lấy `id` từ URL qua `useParams()`, tìm thông tin dự án tương ứng để hiển thị.
- [x] Implement breadcrumb điều hướng và nút "Quay lại" (`useNavigate(-1)`).
- [x] Vẽ sơ đồ Route Map lưu vào `docs/week06-routemap.png`.

**SavedProjectsPage [C]**
- [x] Nhận danh sách dự án đã lưu và hiển thị dạng bảng hoặc list.
- [x] Có nút để hủy lưu/xóa khỏi danh sách, tính toán số lượng dự án.
...
- [x] Tạo `ProjectContext`, provider chứa state `savedProjects`, các hàm `saveProject`, `unsaveProject`.
- [x] Tạo custom hook `useProject()` để components dễ dàng gọi dữ liệu.
- [x] Viết `useEffect` để cập nhật `document.title` dựa trên số lượng dự án đã lưu (ví dụ: `(3) FJMS Marketplace`). Có cleanup function.

**ThemeContext [B]**
- [x] Xây dựng context đổi giao diện Sáng/Tối.
- [x] Sử dụng `useEffect` đồng bộ theme đã chọn vào `localStorage`.

**useEffect Fetch Mock & useRef & Refactor [C]**
- [x] Viết `useEffect` trong `ProjectListPage` mô phỏng fetch API với `setTimeout` 800ms, có clean up timer.
- [x] Dùng `useRef` tự động focus vào ô tìm kiếm sau khi dữ liệu load xong.
- [x] Refactor Header, ProjectCard để lấy trực tiếp dữ liệu từ Context, loại bỏ props drilling.
...
**Setup Service [A]**
- [x] Tạo `src/services/projectService.js` sử dụng `axios.create` cấu hình Base URL và timeout.
- [x] Viết đầy đủ 5 phương thức CRUD: `getProjects`, `getProjectById`, `createProject`, `updateProject`, `deleteProject`.

**Tích hợp Fetching & UI States [B]**
- [x] Gọi API trong `ProjectListPage` bằng async/await bên trong `useEffect`.
- [x] Quản lý 3 trạng thái: `loading` (hiển thị Spinner), `error` (hiển thị alert có nút retry), và `success` (hiển thị dữ liệu).
- [x] Lập bảng danh sách các API endpoints (Method, URL, Body, Response).

**Admin CRUD Management [C]**
- [x] Thiết kế trang quản lý dự án dạng bảng Bootstrap/Tailwind.
- [x] Tạo Modal chứa Form thêm mới và chỉnh sửa dự án. Validate dữ liệu trước khi gửi.
- [x] Thực hiện chức năng xóa dự án với confirm dialog và alert thông báo thành công.
...
**useFetch.js [A]**
- [x] Viết custom hook `useFetch(url, params)` trả về `{ data, loading, error, refetch }`.
- [x] Dùng `useCallback` tránh tạo lại hàm gây lặp vô hạn.

**useDebounce.js & Optimize Search [B]**
- [x] Viết hook `useDebounce(value, delay)`.
- [x] Áp dụng vào thanh tìm kiếm dự án. Đảm bảo người dùng ngừng gõ 500ms mới kích hoạt API request.

**useLocalStorage.js & Wishlist [C]**
- [x] Viết hook `useLocalStorage(key, initialValue)` lưu giữ trạng thái đồng bộ với local storage.
- [x] Dùng hook này quản lý danh sách dự án yêu thích (Wishlist).
- [x] Kiểm tra toàn bộ code trong dự án đảm bảo tuân thủ nghiêm ngặt Rules of Hooks (không gọi hook trong vòng lặp, điều kiện).
...
- [x] Cài đặt `@reduxjs/toolkit` và `react-redux`.
- [x] Tạo `src/store/projectSlice.js` chứa state và các reducers ứng tuyển/lưu dự án.
- [x] Tích hợp Redux Store vào `main.jsx` bằng `<Provider>`.
- [x] Migrate component Header, ProjectCard, SavedProjectsPage sang dùng `useSelector` và `useDispatch`.
- [x] Chụp màn hình hoạt động của Redux DevTools (Time-travel debugging).

**Tailwind CSS Migration (Branch: `feature/tailwind`) [B]**
- [x] Cấu hình Tailwind CSS trong Vite.
- [x] Tái cấu trúc ProjectCard và Banner bằng các utility classes của Tailwind (thay thế hoàn toàn Bootstrap tại các file này).

**Lazy Loading & Tối ưu hiệu năng (Branch: `feature/perf`) [C]**
- [x] Áp dụng `React.lazy` và `Suspense` chia nhỏ bundle code theo Route (Route-based code splitting).
- [x] Thêm `loading="lazy"` cho ảnh logo/banner của dự án.
- [x] Viết prefetch chunk file khi hover vào các link menu điều hướng.
- [x] Chạy Bundle Analyzer kiểm tra kích thước bundle và tối ưu hóa.

**Build & Deploy Vercel (Branch: `main`) [A + ALL]**
- [x] Cấu hình `vercel.json` phục vụ rewrite URL tránh lỗi 404 khi F5.
- [x] Deploy lên Vercel, kiểm tra chạy thử trên thiết bị di động thật.
- [x] Tạo slide (8-10 trang) và quay video demo sản phẩm (3-5 phút).
